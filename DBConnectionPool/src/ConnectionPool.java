...
public class ConnectionPool {
	...
	private int maxConnections = 30; // 连接池最大的大小
	...
	public ConnectionPool() {
		...
		this.dbUrl = "jdbc:mysql://127.0.0.1:3308/test?useUnicode=true&characterEncoding=utf8";
	...
	public synchronized Connection getConnection() throws SQLException {
		...
		while (conn == null) {
			// 等一会再试
			wait(250);
			conn = getFreeConnection(); // 重新再试，直到获得可用的连接，假如getFreeConnection() 返回的为 null 则表明创建一批连接后也不可获得可用连接
		}
	...
	private boolean testConnection(Connection conn) {
		try {
			// 判定测试表是否存在
			if (testTable.equals("")) {
				// 假如测试表为空，试着使用此连接的 setAutoCommit() 方法来判定连接否可用（此方法只在部分数据库可用，假如不可用 , 抛出异常）。注意：使用测试表的方法更可靠。
